...
export default function AdminDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [memRes, hymnRes, rehRes, evtRes, gapRes] = await Promise.all([
      supabase.from('user_profiles').select('voice_part, voice_confirmed, status'),
    ...
    for (const m of members) {
      if (m.voice_part) byVoice[m.voice_part] = (byVoice[m.voice_part] || 0) + 1;
    }
    setStats({
              ...
              return (
                <li key={v}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className="font-medium">{VOICE_LABELS[v]}</span>
                    <span className="text-muted-foreground">{count} membre{count > 1 ? 's' : ''}</span>
                  </div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden" role="img" aria-label={`${VOICE_LABELS[v]} : ${pct}%`}>
