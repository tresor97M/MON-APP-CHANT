...
export default function AskQuestionPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-5 bg-white rounded-2xl border border-border p-6 shadow-sm">
        {error && <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-xs text-red-600 font-medium">{error}</div>}
        ...
        <div className="space-y-1.5">
          ...
          <input type="text" value={title} onChange={e => setTitle(e.target.value)} required
            ...
            className="w-full px-4 py-3 rounded-xl border border-border bg-muted/30 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all placeholder:text-muted-foreground"
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <select value={category} onChange={e => setCategory(e.target.value)}
            className="w-full px-4 py-3 rounded-xl border border-border bg-muted/30 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all">
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-foreground">
            Tags <span className="text-muted-foreground font-normal">({lang === 'fr' ? 'max 5' : 'max 5'})</span>
          </label>
          <div className="flex flex-wrap items-center gap-2 p-3 rounded-xl border border-border bg-muted/30 min-h-[44px]">
            {tags.map(t => (
              <span key={t} className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold">
                {t}
            ...
            {tags.length < 5 && (
              <input type="text" value={tagInput}
                ...
                className="flex-1 min-w-[80px] bg-transparent text-xs outline-none text-foreground placeholder:text-muted-foreground"
              />
            )}
          </div>
          <div className="flex flex-wrap gap-1.5">
            {TAGS.filter(t => !tags.includes(t)).map(t => (
              <button key={t} type="button" onClick={() => addTag(t)}
                className="px-2.5 py-1 rounded-full border border-border text-[10px] font-semibold text-muted-foreground hover:border-primary hover:text-primary transition-colors">
                + {t}
              </button>
            ))}
          </div>
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={loading}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-bold hover:opacity-90 transition-opacity shadow-sm disabled:opacity-60">
